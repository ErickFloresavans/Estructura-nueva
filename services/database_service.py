        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    query = """
                        SELECT id, ItemName, ItemCode, OnHand, DfltWH, IsCommited, Updated
                    ...
                    # Enriquecer cada resultado con información adicional
                    parts_with_info = []
                    for part in results:
                        part_dict = dict(part)
                        
                        # Agregar información de disponibilidad
                        availability = self._get_part_availability(part['id'])
                        part_dict['availability'] = availability
                        
                        # Agregar información de status
                        status = self._get_part_status(part['id'])
                        part_dict['status'] = status
                        
                        parts_with_info.append(part_dict)

                    print(f"✅ Encontradas {len(parts_with_info)} piezas para '{search_term}'")
                    return parts_with_info

        except Exception as e:
            print(f"❌ Error en search_parts: {e}")
            return []
